...
export function SearchableSelect({ allowClear = false, allowCustom = true, clearLabel = "全部", label, onChange, options, placeholder, value }: { allowClear?: boolean; allowCustom?: boolean; clearLabel?: string; label: string; onChange: (value: string) => void; options: string[]; placeholder: string; value: string }) {
  ...
  const exactMatch = matches.includes(query.trim());
  const disabled = !query.trim() || (!allowCustom && !exactMatch);
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><Pressable onPress={() => { setQuery(value); setOpen(true); }} style={styles.control}><Text style={value ? styles.value : styles.placeholder}>{value || (allowClear ? clearLabel : placeholder)}</Text><MaterialCommunityIcons color={colors.textMuted} name="chevron-down" size={22} /></Pressable>
    <Modal animationType="fade" onRequestClose={() => setOpen(false)} transparent visible={open}><View style={styles.backdrop}><View style={styles.card}><Text style={styles.title}>{label}</Text><TextInput autoFocus onChangeText={setQuery} placeholder={placeholder} placeholderTextColor={colors.textMuted} style={styles.input} value={query} /><ScrollView nestedScrollEnabled showsVerticalScrollIndicator style={styles.options}>{allowClear ? <Pressable onPress={() => choose("")} style={styles.option}><Text style={styles.optionText}>{clearLabel}</Text></Pressable> : null}{matches.map((item) => <Pressable key={item} onPress={() => choose(item)} style={styles.option}><Text style={styles.optionText}>{item}</Text></Pressable>)}</ScrollView><View style={styles.actions}><Pressable onPress={() => setOpen(false)} style={styles.cancel}><Text style={styles.cancelText}>取消</Text></Pressable><Pressable disabled={disabled} onPress={() => choose(query)} style={[styles.submit, disabled && styles.submitDisabled]}><Text style={styles.submitText}>{exactMatch ? "選擇" : "使用新值"}</Text></Pressable></View></View></View></Modal>
  </View>;
}
