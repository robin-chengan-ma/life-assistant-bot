"""內建指令與 Owner 對話式設定流程（對應 docs/specs/platform-auth/SPEC.md FR-4～FR-6）。"""
...
def start_set_invite_codes(state_store: ConversationStateStore, telegram_user_id: int) -> str:
    ...
    state_store.set(telegram_user_id, {"step": "awaiting_role"})
...
def handle_set_invite_codes_step(
    ...
    if step == "awaiting_role":
        ...
        state_store.set(telegram_user_id, {"step": "awaiting_code", "role": text})
