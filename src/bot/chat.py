...
def handle_chat_message(
    ...
    prompt = _build_prompt(context, long_memory, text, pending_question)
    ...
    if pending_question is not None and _SAVE_MARKER in reply_text:
        ...
        if pending_question is not None:
            ...
            # 不該再讓舊的 pending_user_knowledge 狀態留在原地卡住下一輪對話。
            state_store.clear(telegram_user_id)
...
def _build_prompt(
    context: dict, long_memory: str, user_message: str, pending_question: str | None = None
) -> str:
    ...
    return (
        ...
        "使用者問到人名時，如果打的名字跟知識庫裡某個人名非常相似（可能是打字誤植、同音字、形似字，"
        "例如「凱」打成「鎧」），先合理假設是打字誤植、當作在問知識庫裡最相近的那個人，不要因為一兩個字"
        "不同就直接說不知道，可以在回答時順便確認你理解的是哪位；"
