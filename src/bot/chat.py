...
2026-08-02 追加修正（Robin 回報「打了『我要載妹妹到水里』完全不理我」，實際根因）：
`pending_question`／`confirming_question` 特別狀況區塊原本的 case (3)／(2)（全新無關內容）
文字寫成「使用者其實是在問一個新的、跟上面那題無關的『問題』」，用詞侷限在「問題」，導致使用者
講的是一句「陳述/新需求」而不是疑問句時（例如本例的「我要載妹妹到水里」，這不是在問問題，而是
在描述一件新的待辦事項），模型容易誤判成案例 (2)（拒絕/跳過），因為只剩兩個選項看起來比較像。
修正：把 case (3)／(2) 的措辭從「問一個新問題」放寬為「除了以上情況以外的任何內容（不論是問題
還是單純的新陳述/新需求）」，讓它成為真正的 catch-all，同時把「拒絕/跳過」那一項收緊為必須是
「明確的拒絕/跳過語句」，避免模型把無關新內容硬套進拒絕分類。
"""
...
def _build_prompt(
    ...
    if pending_question is not None:
        pending_block = (
            ...
            "(2) 使用者明確表示不想提供、不用記錄、想跳過（例如「不用了」「算了」「不用紀錄啦」這種"
            "明確的拒絕/跳過語句）→ "
            f"整則回覆只能輸出這個固定標記，不要輸出其他任何文字：「{_DECLINE_MARKER}」\n"
            "(3) 除了以上兩種情況以外的任何內容——不管是問了一個新問題，還是單純講了一件跟上面那題"
            "完全無關的新事情/新需求（例如描述一件新的待辦事項、聊到新話題），只要不是在提供答案、"
            "也不是明確表示拒絕/跳過，就一律屬於這一種、不可以硬套用 (2) → 完全忽略這個特別狀況，"
            "把這則新訊息當成一則全新的一般聊天訊息，依照下方所有規則正常回答（該不知道就不知道、"
            "該回答就回答、該偵測待辦意圖就偵測），不要輸出任何標記，也不要假設使用者在回答上一題。\n\n"
        )
        ...
        pending_block = (
            ...
            "(2) 除此之外的任何內容——使用者否認你的猜測、或講了跟這件事完全無關的新事情/新需求"
            "（不論是問題還是單純的新陳述）→ 完全忽略這個特別狀況，把這則新訊息當成一則"
