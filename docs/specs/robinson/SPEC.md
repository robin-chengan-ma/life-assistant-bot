...
- [ ] FR-31：使用者以自然語言描述「什麼時候要做什麼事」，Robinson 解析後記錄
- [ ] FR-32：推播時機：使用者主動查詢時、每日 08:00 固定推播、預計處理時間前 30 分鐘提醒
...
- [ ] FR-46：身體數據：目標設定、身高（初始設定，變動才修正）、體重（有量才記）、自動計算 BMI 並附標準說明
