...
- [x] FR-22（**2026-08-07 完成，見 Step 3.1**）：每日固定時間（預設 08:00，可改）自動推播技能成長摘要——`src/bot/skill_growth.check_and_push_daily_digest()` 借用 `/healthz` 既有的 10 分鐘 cron 頻率，只在台灣時間 08 點這個小時內執行，靠 `users.skill_growth_pushed_on`（比照 `todos.daily_pushed_on` 慣例）避免同一天內重複推播；「可改」（自訂推播時間）留待 Robin 實際有需求時再擴充，目前固定 08:00，比照既有的 `todo`／`notifications` 每日摘要模組的既有慣例（同樣固定時間、暫無自訂機制）
- [x] FR-23（**2026-08-07 完成，見 Step 3.1、submodules-core SPEC.md ADR-11 追記／ADR-14**）：每日重點技術分享（開關）：擷取 Gmail 昨日 TLDR 電子報 + IThome / TechCrunch 昨日新聞，經 Gemini 產出中文重點摘要與總結分享（依 NFR-11，以來源日期避免重複摘要已處理過的電子報/新聞）。實作細節：①TLDR 電子報用 `submodules/email` 新增的 `fetch_yesterday_emails_from_domain()`（IMAP，寄件者網域比對 `tldrnewsletter.com`，經 AskUserQuestion 確認）②IThome／TechCrunch 用新增的 `submodules/newsfeed`（RSS Feed，`requests`＋標準函式庫 `xml.etree.ElementTree`）③三個來源任一失敗只記 log、視為當日無內容，不影響其他來源與整體推播④三個來源都沒內容時仍推播固定的「今天沒有新內容」訊息，不靜默跳過（NFR-10）⑤去重用「來源日期」（每天只處理「昨天」這一天固定範圍）＋`users.skill_growth_pushed_on`（今天是否已推播過）兩層機制，不需要額外的內容雜湊表⑥`skill_growth` 功能開關（`owner_only=True`）關閉時跳過，不消耗 Gemini API 額度⑦Gemini 呼叫用獨立的 `GEMINI_API_SKILL_GROWTH_KEY`，避免佔用其他功能既有 Key 的配額（Robin 需自行在 AI Studio 申請這把新 Key 並設定到部署環境，`.env.example` 已列出，未設定時這個排程檢查會優雅跳過、不影響其他功能）
