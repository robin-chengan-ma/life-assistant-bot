...
> **2026-08-12 依 `codex.md` 開發紀錄同步**：Step 4.4／4.5 已由 Placeholder 進度大幅推進為實作中，FR-64（唯讀分析＋首頁）、FR-64a（體態/飲食記錄，**設計已由藍牙體重計改為手動輸入，見下方條目**）、FR-65（登入與 Token）、FR-67／FR-67a／FR-67b（選單與權限矩陣）、FR-68（個人基本資訊）、FR-72（APP 設定）皆已完成後端 API＋Mobile 頁面＋自動化測試並經 Robin 本機／實機驗證，下方逐項勾選並補充實作摘要；詳細逐輪異動見 `codex.md`，里程碑總覽見 PROGRESS.md。FR-69／FR-70／FR-71（目標與指標設定／功能開關／排程設定）**尚未開工**，維持 Placeholder。本輪另發現一個**尚未正式排入規格**的新功能方向（收藏清單／探索地圖／成果展示），已完成技術可行性 POC 但產品規則尚未定案，見本節最後新增的 FR-73（Placeholder）。

- [x] FR-64（**2026-08-09 範疇修正，見 ADR-28，supersede 原「唯讀 BI Dashboard」定位**）：App 採「唯讀分析頁面＋可編輯設定頁面」雙軌設計。唯讀分析頁面：將記帳、體態管理等模組的資料計算為圖表（消費圓餅圖、體重/運動趨勢折線圖等）與篩選介面呈現（取代原 FR-54 的 Notion 方案）。可編輯設定頁面（**2026-08-09 新增範疇**）：目標與指標設定（FR-69）、功能開關（FR-70）、排程設定（FR-71，僅 Robin）、APP設定（FR-72），開放使用者直接在 App 端調整自己的目標/開關/排程，不必再透過 Telegram 對話流程逐步反問設定。**寫入分工原則**：App 端的所有寫入操作一律複用既有 service 層函式（例如體態目標寫入直接呼叫 `body.py` 既有的目標建立/更新函式），不重複開發第二套業務邏輯；App 與 Telegram 兩個入口最終寫入同一批資料表，互為對等入口而非誰取代誰。**維持不變的部分**：日常「記一筆」性質的高頻操作（記一筆支出、記一次體重、新增一則待辦）語意上更適合 Telegram 的自然語言對話（一句話講完，不用切換 App 填表單），本次範疇擴大僅開放「目標/開關/排程」這類低頻率、結構化的設定類操作進 App，不是把所有 CRUD 都搬進 App。**唯一已知例外（2026-08-09，見 [mobile-app SPEC.md ADR-1](../mobile-app/SPEC.md)）**：客訴回饋頁面裡的「系統錯誤回報」區塊（FR-19j）破例支援「解法」單一欄位可編輯，其餘欄位仍唯讀；同一頁的「使用者客訴」區塊（FR-60～FR-63）維持完全唯讀，不受此例外影響，範圍極小、不影響本條整體分工原則。**2026-08-12 實作完成**：唯讀分析頁面（待辦事項／體態分析／記帳分析／心情趨勢／求職分析／考試成績／技術分享／客訴回饋）與可編輯設定頁面（FR-68／FR-72 已完成，FR-69／FR-70／FR-71 尚未開工）皆已依 mobile-app/SPEC.md 逐頁完成，`/api/app/*` 圖表就緒 JSON API＋Mobile 頁面渲染＋自動化測試皆已通過；待辦事項與重要日子另外新增「日期區間」設計（原規劃為單日）、各分析頁日期選擇器統一改為共用既有待辦行事曆的中華民國政府行事曆資料，詳見 `codex.md` 與 mobile-app/SPEC.md 對應章節。
- [x] FR-64a（2026-08-08 新增，Placeholder；**2026-08-09 因 FR-64 範疇擴大微調最後一句措辭；2026-08-12 依 `codex.md` 同步最終實作設計，見下方「2026-08-11 設計變更」**）：藍牙體重計整合——Robin 已購入支援藍牙廣播（BLE Advertisement）的體重計，並用 nRF Connect for Mobile 實測確認可從掃描結果的 Manufacturer Data（廠商資料）取得量測後的體重值。App 端新增「開始量測」按鈕（**2026-08-10 統一按鈕文案措辭**：先前誤寫成「開始測量」，比照 [mobile-app SPEC.md](../mobile-app/SPEC.md) 首頁「體重紀錄」卡片同一顆按鈕的文案統一為「開始量測」），點擊後啟動藍牙掃描，10 秒內偵測到體重值則顯示並記錄；10 秒內未偵測到則顯示「未取得您的體重值」，不記錄任何資料。**2026-08-10 新增**：首頁「體重紀錄」卡片（見 mobile-app SPEC.md）內建同一顆「開始量測」按鈕，是這個藍牙量測流程在 App 內的主要入口，不需要先進體態分析頁才能觸發。**體重紀錄維持雙入口**：人不在家、沒帶體重計時，同樣可以直接在 Telegram 手動輸入體重值（既有 FR-46 `/log_weight` 流程不變）；兩種入口最終都寫入同一張 `body_weight_logs`，App 端這支寫入 API 沿用「後端算好結果、App 只負責渲染/呼叫」的既有分層原則，實際寫入邏輯復用現有 `body.create_weight_log()` 等既有服務層函式，不另外複製一份業務邏輯（**2026-08-09 更新**：FR-64 範疇擴大後，此例外不再是唯一的 App 寫入入口，但「複用既有 service 層、不重複開發業務邏輯」的分工原則不變，見 FR-64／ADR-28）。

  **2026-08-11 設計變更（已推翻上方藍牙描述，見 `codex.md`）**：實機驗證發現 Yoda1 體重計為「不可連線」被動廣播裝置，React Native 原生 `react-native-ble-plx` 雖可解析（Big Endian、除以 100，見下方原始技術細節），但需要 Development Build、不支援 Expo Go／Web；進一步嘗試以 PWA＋Bluefy／Web Bluetooth `watchAdvertisements()` 取代原生 BLE 的相容性 POC，實機測試確認瀏覽器完全收不到廣播事件，方案不可行。**Robin 最終決策：全面移除藍牙功能**，Android／iOS／Web 三平台皆不再掃描 Yoda1、不要求藍牙權限。改為 App 內「記錄一下」按鈕（原「開始量測」，圖示由藍牙改鉛筆）開啟 Modal 手動輸入體重，輸入限制數字＋一位小數、可記錄範圍 `40.0～150.0 kg`（前後端雙重驗證，超出範圍拒絕儲存），四捨五入至小數點後一位，二次確認「XX.X 公斤是否正確？」後才呼叫既有 `POST /api/app/body/weight-logs`（沿用 `body.create_weight_log()`，寫入同一張 `body_weight_logs`，與 Telegram `/log_weight` 為對等入口不變）。下方「藍牙資料解析規格」與「三項技術細節設計提案」段落**保留作為歷史紀錄／未來若重新評估藍牙方案的參考**，目前實際產品不使用。

  **2026-08-12 新增（FR-64a 飲食記錄，同一 FR 編號涵蓋）**：首頁「記錄今日飲食」支援文字輸入／拍照／從相簿選擇；照片先呼叫 Gemini 圖片辨識轉換為可編輯的飲食描述文字（辨識不確定的項目會標註），使用者確認描述後才呼叫營養估算 API 並寫入資料庫，避免重複呼叫 Gemini；圖片本身不落地存檔，僅用於單次辨識。
  ...
  **三項技術細節設計提案（2026-08-10 新增，Claude 提出、Robin 已同意採納，Phase 4 開工時依此展開）**：
  1. **辨識「這台特定體重計」的廣播封包**：分兩層過濾，避免掃到附近其他藍牙裝置。第一層先用 Manufacturer Data 開頭的廠商識別碼（company identifier，通常是封包最前面 2 bytes，跟現有解析邏輯取的「索引 2、3 位元組」是同一包資料裡不同位置）粗篩掉明顯不相關的裝置；第二層是首次使用時的「一次性綁定」流程——使用者從掃描結果手動選出正確的那台體重計，App 把該裝置的藍牙識別碼（MAC address／iOS 上對應的 peripheral UUID）存起來（存在裝置本機即可，不需要存後端，同一台手機才需要記得），之後每次「開始量測」直接鎖定這個識別碼掃描，不再需要使用者每次手動選。
  2. **掃描逾時之外的例外處理**：藍牙未開啟——量測前先檢查手機藍牙開關狀態（React Native BLE 函式庫皆有狀態監聽 API），未開啟時直接提示「請先開啟手機藍牙」，不進入掃描流程；權限未授權——首次使用觸發系統原生的藍牙／位置權限請求（Android 12+ 藍牙掃描額外需要位置權限），使用者拒絕時提示「請至系統設定開啟藍牙權限」並提供直接跳轉系統設定頁的捷徑；使用者中途離開頁面或切到背景——立即中斷掃描，不做背景常駐（這顆按鈕本身設計成「顯式觸發、有限時間視窗」的一次性動作，不需要為了背景掃描額外耗電）。
  3. **App 端寫入 API 路由設計與身分驗證**：沿用 FR-64「技術細節補充」既有的 `/api/app/*` 路由群組與 JWT 身分驗證慣例，新增 `POST /api/app/body/weight`，request body 帶 App 端已解析好的公斤數（例如 `{"weight_kg": 68.5}`）。**這支 API 是 FR-64「後端算好結果、App 只負責渲染/呼叫」原則的刻意例外**：BLE 封包解析必須發生在手機端（後端沒有能力遠端讀取手機的藍牙掃描結果），所以反過來是「App 端算好公斤數、後端只負責驗證身分後存檔」；後端仍然只呼叫既有 `body.create_weight_log()` 寫入，不重複開發業務邏輯，跟既有分工原則的精神一致，只是這一支 API 的計算工作方向相反。
- [x] FR-65（**2026-08-09 全面改寫，見 ADR-28，取代原「Token-based」登入設計；2026-08-10 實作完成**）：App 登入頁面欄位為「使用者ID」與「密碼」（各自可用眼睛圖示切換顯示/隱藏），取代原本規劃的 `APP Access Token` 免密碼機制；App 名稱為「羅賓森」，App icon 使用 `docs/profile/Robinson.png`（Robin 大頭貼，**永久禁止刪除**，見 SPEC.md「重要資產」章節）。**實作摘要**：`AppAuthService`（bcrypt 密碼雜湊、HS256 JWT Access Token 30 分鐘效期、opaque Refresh Token 30 天效期＋每次 refresh 皆撤銷舊發新 rolling refresh）、`/api/app/auth/*`（login／forgot-password／refresh／logout／me）、Expo SecureStore 保存 Refresh Token、`AuthContext` 自動還原登入態；2026-08-11 修正登入頁使用者 ID 輸入 350ms 停止後自動預先辨識、白畫面 Hook 順序 bug；2026-08-12 修正 Web 預覽 API Base URL 改用 `window.location.origin` 避免綁死過期 Tunnel 網址。
  - [x] FR-65a：登入互動與提示文案 —— 使用者ID 辨識失敗跳出「未成功辨識您的身份」；密碼錯誤跳出「密碼錯誤」；登入成功跳出「驗證成功，請稍候！」後導向首頁（**取代原 FR-65a／FR-65b 的「Robin 少填一項」設計**：密碼機制下沒有可比照「稱謂」省略的欄位，Robin 與一般使用者登入欄位數量一致，身分差異改由 FR-67a 的選單可見性處理，不在登入階段區分）
  - [x] FR-65b（**技術決策，見 ADR-28，2026-08-09 Robin 確認採納；2026-08-10 實作完成**）：忘記密碼 —— 點擊「忘記密碼」後，系統產生一組新的隨機密碼、覆蓋原密碼並透過 Telegram 私訊發送給該使用者（是「重設」為新密碼，不是「找回」原密碼）；使用者登入後可在 FR-72 APP設定自行改成想要的密碼。**理由**：忘記密碼流程要求系統能把可用密碼交給使用者，代表系統必須存有可用的密碼值；業界標準是密碼只存單向雜湊（見 FR-65d），系統本身也無法還原明碼，因此只能重設為新密碼，這是資安最佳實踐與「透過 Telegram 拿到能登入的密碼」原始需求之間風險最低的折衷做法。
  - [x] FR-65c（**技術決策，見 ADR-28，2026-08-09 Robin 確認採納 30 天效期；2026-08-10 實作完成**）：保持登入 —— 勾選後，裝置本機以 Expo SecureStore 儲存一組長效 Refresh Token（有效期 30 天，超過需重新登入），下次開啟 App 免登入直接進首頁；Refresh Token 到期前，App 背景自動用它換發新的 Access Token 與 Refresh Token（Rolling Refresh）。**已知限制（MVP 不做，留待未來有需要再展開）**：不提供「登出所有裝置」／「查看目前登入裝置清單」等進階裝置管理功能；裝置遺失時暫時只能靠 Robin 手動於資料庫端撤銷該使用者的 Refresh Token 補救。
  - [x] FR-65d（**技術決策，見 ADR-28；2026-08-10 實作完成，修改密碼流程見 FR-72**）：密碼儲存安全性 —— 密碼一律以 bcrypt（或同等強度的 argon2）單向雜湊後存入 `users.password_hash`，資料庫與程式碼任何地方都不保留明碼；FR-68 個人基本資訊頁面因此不顯示密碼明碼，改顯示「密碼最後修改時間」，並提供「修改密碼」按鈕導向 FR-72 的修改密碼流程。
- [x] FR-67（2026-08-09 新增，見 ADR-28；**2026-08-11 實作完成**）：App 首頁佈局左上角為選單（漢堡）圖示，用以導覽各功能分析頁面，右上角為個人頭像圖示（無需個人化客製，僅提供男/女兩種通用樣式依 `users` 既有角色欄位挑選即可），點擊後展開下拉選單：
  - [x] FR-67a：權限矩陣與選單顯示順序 —— 一般使用者（所有已綁定家人）可見：待辦事項、體態分析、記帳分析、心情趨勢；Robin 額外可見：求職分析、考試成績、技術分享、客訴回饋（其餘使用者看不到這四項，比照既有 Telegram 端 `owner_only` 功能開關慣例，見 FR-2）。左上角選單固定顯示順序：待辦事項、體態分析、記帳分析、心情趨勢、求職分析（Robin only）、考試成績（Robin only）、技術分享（Robin only）、客訴回饋（Robin only）；使用者關閉中的功能（FR-2 功能開關為關）該項目不顯示於選單，與 Telegram 端「功能關閉時不回應該類對話」的既有原則一致。
  - [x] FR-67b：右上角頭像下拉選單結構 —— 依序為「個人基本資訊」（FR-68，唯讀）、「目標與指標設定」（FR-69，可編輯，**尚未開工**）、「功能開關」（FR-70，可編輯，**尚未開工**）、「排程設定」（FR-71，可編輯，**僅 Robin 可見，尚未開工**）、「APP設定」（FR-72，可編輯）、「登出」。**2026-08-11／2026-08-12 實作完成**：選單項目視覺一致性、間距與深色模式對比度已修正；FR-69／FR-70／FR-71 尚未開工，選單項目暫不顯示或顯示為未來項目，實際呈現以 mobile-app/SPEC.md 為準。
- [x] FR-68（2026-08-09 新增，見 ADR-28；**2026-08-11 實作完成**）：個人基本資訊頁面（唯讀）—— 顯示「暱稱」、「使用者ID」、「密碼最後修改時間」（**2026-08-09 修正**：不顯示密碼明碼，理由見 FR-65d）、「角色」；一併顯示既有的體態量測基準值「身高」／「腰圍」（`users.height_cm`／`waist_cm`，**2026-08-09 缺口盤點新增**：這兩者是量測基準值而非目標，性質上更適合放在個人基本資訊而非 FR-69 目標與指標設定，Robin 原描述遺漏）。
...
- [x] FR-72（2026-08-09 新增，見 ADR-28；**2026-08-12 實作完成**）：APP設定頁面（可編輯）—— 深色／淺色模式切換、字體大小（小／中／大）、隱私數字遮罩（選擇性，開啟後金額/體重等敏感數字於畫面上以 `***` 等方式遮蔽，需要時再點擊顯示）；此頁面同時作為 FR-65d「修改密碼」功能的入口（輸入舊密碼＋新密碼＋確認新密碼）。上述設定值皆為使用者個人偏好，不影響其他使用者。**儲存方式（2026-08-10 定案）**：存進後端 `users` 表新增偏好欄位（不存裝置本機），這樣使用者換裝置或重灌 App 登入後設定仍會保留，不需要每台裝置各自重新設定一次；實際欄位名稱與型別（例如 `theme_preference`／`font_size_preference`／`privacy_mask_enabled`）依 ADR-10 流程於 Phase 4 開工時提出 migration（**2026-08-12 已依此完成 migration `0067_add_mobile_app_preferences.sql`**）。個人選單間距與深色模式對比度、今日紀錄視窗深色配色已於 2026-08-12 修正。

- [ ] FR-73（**2026-08-12 新增，Placeholder，依 `codex.md` 技術可行性 POC 記錄提出，尚未定案、尚未開工**）：收藏清單／探索地圖／成果展示 —— 首頁新增收藏清單資料結構與新入口，另外針對「探索地圖」完成跨平台技術驗證：地圖採 Leaflet 1.9.4＋OpenStreetMap 圖磚（Expo Web 交付，iOS／Android 皆透過瀏覽器使用，不需原生 Development Build 或上架），未採用官方 Expo Maps（仍是 alpha、不支援 Expo Go、Android 需另外設定 Google Maps SDK）；地址搜尋若未來串接 OpenStreetMap Nominatim，須遵守其公開服務限制（僅使用者主動按下搜尋才呼叫、不做即時自動完成、整個 App 每秒最多一次請求、需提供識別資訊與署名、快取結果、保留日後替換搜尋服務的能力），正式功能地圖須顯示 OpenStreetMap 著作權署名。**本條目前僅完成技術可行性 POC（`mobile/app/map-poc.tsx`，假資料）**，尚未確認完整產品規則（收藏清單資料結構定案範圍、探索地圖與收藏清單的關聯、成果展示的呈現方式），需另外走一輪規格確認流程後才能正式展開，不阻塞現有 Step 4.4／4.5 其餘項目。
