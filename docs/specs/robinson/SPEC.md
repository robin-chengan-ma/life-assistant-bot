...
| 層級 | 服務 | 角色 |
| --- | --- | --- |
| 前台 | Telegram Bot | 唯一使用者介面（文字 + 語音） |
| AI 層 | Gemini API（`gemini-flash-latest`，共用兩把 Token） | 一把跑對話、一把跑圖像/證照題目解析 |
| 資料層 | Neon PostgreSQL | 結構化資料（使用者、知識庫、待辦、記帳…） |
| 資料層 | Google Drive | 靜態圖像（含證照題目截圖） |
...
- [ ] FR-17：僅接受「證照題目」用途的圖片，其餘圖片一律拒收並說明限制
...
- [ ] FR-25：TOEIC 為第一個題庫，每次出題預設 1 題聽力 + 2 題填空 + 3 題單字英翻中，採「雙軌混合架構」產生題目；未來可套用同模板擴充 AWS / GCP 等其他證照：
  ...
  - [ ] FR-25b：軌道一檔名比對規則 — 若為已分拆的音檔，系統直接比對圖檔與音檔的檔名規則（例：`20260729_test01_q01.png` 對應 `20260729_test01_q01.mp3`），比對成功即整合為一筆完整題目；若為整包大型 MP3，Robinson 呼叫 Gemini 進行語音轉文字（STT）與時間軸標籤比對，自動依題目段落切割為獨立小音檔（依專案規範，語音辨識一律用 Gemini，不使用 Whisper 等其他模型）
...
- [ ] NFR-5：安全 — 敏感金鑰（Telegram Token、Notion API Key、Gemini API Key、Neon 連線字串、Google Service Account JSON、Gmail 密碼、GitHub Personal Access Token、YouTube Data API Key）一律透過 `.env` 管理，不進版控
...
- [ ] Step 3.2：TOEIC 雙軌題庫 Pipeline（FR-24、FR-25a～FR-25f）—— 軌道一拍照/音檔入庫（Gemini Vision + Gemini STT）、軌道二 Gemini 單字題即時生成、週排程去重
...
3. 羅賓森目前僅會根據「已知知識」做出回答。若需要即時上網查詢的資訊（例如：「國道有沒有塞車」、「下午天氣如何」等），請先自行搜尋。若是固定知識（例如：「威靈頓牛排食譜」或「澎湖行程規劃」），您可以把答案提供給羅賓森，他會幫忙記錄下來、學習更多新知識喔！
...
**變更紀錄**：2026-07-30 開頭語句由「🎉 通關密碼驗證成功！歡迎使用羅賓森 AI 服務。」改為「📋 以下是羅賓森的使用須知：」，讓 FR-6d（歡迎訊息）與 FR-55（`/rule`）共用同一份文案時語境都通順；同時補上「我要客訴你」的觸發提示，呼應新增的 FR-60～FR-63 客訴收集功能。
