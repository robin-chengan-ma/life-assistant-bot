...
- [ ] FR-36（**2026-08-09 修正，見 ADR-26 決策 1**：歸屬 Step 4.1，與 FR-33 同一輪對話流程收集，不歸屬 Step 4.2）：記錄使用者 3500 字內個人履歷與未來期望工作內容；**新增兩個結構化欄位輔助 FR-37 契合度評分**：①年資（`years_of_experience`，浮點數，例如 3.5 年，Robin 直接填數字）②期望薪資下限／上限（`expected_salary_min`／`expected_salary_max`，數字），從原本混在自由文字裡的「期望工作敘述」拆出來，不用再靠 LLM 從自由文字猜測
