...
- FR-72a：具明確日期的體態目標（體重／運動／飲食）及考試／證照目標，預設同步為一次性「重要日子」；提前 1 天提醒、通知自己並顯示於待辦行事曆，使用者可在重要日子設定調整提醒與通知對象。目標名稱／日期更新時同步更新且保留既有提醒設定；目標達成、取消或清除日期時停用連動事件。現行記帳只有月份預算，沒有單一完成日期，本階段不納入同步——已實作
...
- FR-74b 行程重要日子同步：新增／編輯行程提供「同步至重要日子」且預設開啟；有起訖日期時建立與行程名稱、日期連動的一次性重要日子。預設提前 1 天、通知自己、顯示於待辦行事曆，使用者可自行調整。後續修改行程保留提醒設定並同步名稱／日期；關閉同步、取消或刪除行程時停用連動事件，恢復有效行程時依同步設定重新啟用。
