...
Robinson 是一個雙前台架構的家庭生活小助手：Telegram Bot 是唯一輸入介面（文字/語音/照片），負責自然語言互動與所有資料 CRUD；Mobile App「羅賓森」是唯讀分析＋可編輯設定的 Rich GUI，補足 Telegram 不擅長的圖表視覺化。Robin 是產品負責人兼管理者，家人透過通關密碼取得使用權限。核心理念是「越少 UI 設定越好」，所有服務一律使用免費方案，AI 能力統一由 Gemini API 依用途分流多把 Key 提供。
...
**概要**：Robinson 採三層式架構（Telegram 前台／Neon+GDrive 資料層／Mobile App 唯讀+設定後台），MVP 依複雜度分 Phase 0～4 逐步交付，資料庫 Schema 一律「先審核後執行」並透過 Migration 檔案自動套用。
...
- Phase 4：求職模組（完成）＋Mobile App（Step 4.4／4.5 為 Placeholder，尚未開工）
