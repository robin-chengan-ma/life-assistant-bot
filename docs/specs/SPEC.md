...
- FR-64c（2026-08-18 定案）：所有 Mobile 目標摘要統一顯示期限、目前進度及進度條；多筆時只從進行中目標挑選最近到期者，同日取最近更新者，全部無期限則取最近更新者，其餘由唯讀「查看全部」呈現。無期限顯示「無期限」，無法量化顯示灰色未定進度與「目前無法計算」。數值改善依基準到目標比例，累積型依目前累積除以目標值，考試依最高正式成績除以目標分數；進度限制 0%～100%，超標另標示「已超越目標」。一次性運動里程碑採未完成 0%／完成 100%，由 Telegram 目標清單明確標記，不從一般運動紀錄猜測。目標新增／編輯／刪除仍由 Telegram 處理，Mobile 只顯示「請前往 Telegram 設定」而不做深層連結
