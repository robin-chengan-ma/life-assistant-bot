...
- [x] FR-10（2026-08-01 新增，2026-08-01 追加修正改為先確認）：`/clean-all-dialog` —— 使用者輸入「我想要刪除所有對話紀錄」或 `/clean-all-dialog` 時，`commands.handle_clean_all_dialog(db, user_id)` 只清除該使用者自己的「對話」：`conversation_logs` 軟刪除（`deleted_at` 設為現在時間，比照既有慣例）＋ `conversation_summaries` 重置為空白摘要（`summary=''`、`summarized_up_to_log_id=0`）；**刻意不動 `knowledge_base`**——這是與 FR-12 的 `/clean-target-dialog`（使用者說「我想刪除有關...的紀錄」時觸發，會連同該主題的知識庫內容一起清除）明確區隔開的不同指令，`/clean-all-dialog` 只處理對話記憶，不處理知識庫。**追加修正起因**：Robin 回報原本觸發詞一送出就直接執行刪除，沒有給使用者反悔機會，違反 FR-16「任何涉及寫入/修改資料庫的操作前一律先與使用者確認」；改為 `commands.start_clean_all_dialog_confirm()` 先查出目前 `conversation_logs` 未刪除筆數告知使用者並反問「確定要清除嗎？」，進入 `pending_clean_all_dialog_confirm` 狀態，下一則訊息由 `commands.handle_clean_all_dialog_confirm_step()` 以單次 LLM 呼叫判斷使用者是「確定」（`CONFIRM`，才呼叫 `handle_clean_all_dialog()` 真正執行）還是「取消」（`CANCEL`，任何無法判斷為確定的回覆一律視為取消，保守優先，不誤刪）
- [x] FR-11（2026-08-01 新增，見 ADR-8）：主動新增知識——使用者主動要求「記住」「新增到知識庫」「幫我存起來」時，`chat._build_prompt()` 依 `is_owner` 給不同的存放範圍規則（見下方 ADR-8），LLM 判斷後在回覆最後加上 `【REQUEST_SAVE】` 標記；`chat.handle_chat_message()` 偵測到標記後，去除標記、把狀態設為 `{"flow": "pending_save_knowledge_confirm", "target_user_id": <int>, "original_request": <str>}`，這一輪**不寫入資料庫**；下一則訊息由 `commands.handle_save_knowledge_confirm_step()` 以單次內部 LLM 呼叫（`DECISION`/`CATEGORY`/`LABEL`/`CONTENT` 固定格式，非使用者可見）判斷確定與否、並整理出分類標籤與內容，`DECISION=CONFIRM` 才呼叫 `knowledge.save_knowledge()` 真正寫入；伺服器端強制：非 Owner 一律 `category="custom"`，不信任模型回傳的分類（見 ADR-8）
- [x] FR-12（2026-08-01 新增，見 ADR-8）：`/clean-target-dialog`——使用者輸入 `/clean-target-dialog <主題>` 或「我想刪除有關 OOO 的紀錄」時，`router.py` 用 regex（`_CLEAN_TARGET_DIALOG_PATTERN`）擷取主題，呼叫 `commands.start_clean_target_dialog_confirm()`：先撈出使用者自己的 `conversation_logs`（未軟刪除）與自己的 `custom` 知識庫作為候選，**只有 Owner 觸發時**才額外把 `general_family`／`general_persona` 也納入候選；候選清單交給單次內部 LLM 呼叫判斷「哪些跟這個主題相關」（回傳編號清單或 `NONE`），沒有候選/沒有匹配時直接告知使用者、不進入確認流程；有匹配則設定 `pending_clean_target_dialog_confirm` 狀態（含 `topic`／`log_ids`／`kb_ids`）並回報「找到 N 則對話紀錄、M 筆知識庫資料...確定要清除嗎？」；下一則訊息由 `commands.handle_clean_target_dialog_confirm_step()` 以單次內部 LLM 呼叫判斷 `CONFIRM`/`CANCEL`，確定才真正執行：`conversation_logs` 軟刪除（比照 FR-10），`knowledge_base` **硬刪除**（`knowledge_base` 沒有 `deleted_at` 欄位）
...
### ADR-8：主動新增知識與 `/clean-target-dialog` —— 共用知識庫寫入/刪除一律限定 Owner，沿用 ADR-6/ADR-7 的兩輪反問確認架構

**背景**：ADR-5/FR-10 的風險表早已標記「目前沒有任何『使用者主動要求，就把資訊寫進知識庫』的機制」是已知缺口，待 Robin 決定範圍後再開發；2026-08-01 Robin 明確指示「主動記知識的功能、/clean-target-dialog API 現在先開發吧」，需要決定兩件事：(1) 主動新增知識時，誰能寫進全家共用的 `general_family`／`general_persona`；(2) `/clean-target-dialog` 誰能刪共用知識庫的資料。

**決策**（2026-08-01 Robin 確認）：
1. **寫入範圍**：`general_persona`／`general_family` 只有 Robin（Owner）能編輯；非 Owner 一律只能寫進自己的 `custom`；Robin 自己也能寫 `custom`（三種存放位置由 LLM 依內容判斷建議，但最終寫入的 `category` 一律由 Python 依 `auth.is_owner()` 現場重新判斷後強制覆蓋，不信任模型的判斷結果——「最後一道伺服器端防線」）。
2. **刪除範圍**：只有 Robin（Owner）觸發 `/clean-target-dialog` 時，候選清單才會納入 `general_family`／`general_persona`；非 Owner 只能檢視、刪除自己的 `conversation_logs` 與自己的 `custom` 知識庫。
3. **分類/標籤**：新增知識時額外反問並記錄一個簡短分類/標籤（例如「SOP」「食譜」「行程」），寫進新增的 `knowledge_base.label` 欄位（`0012_add_label_to_knowledge_base.sql`，允許 NULL，既有資料不回填），方便之後依主題查找，也供 `/clean-target-dialog` 判斷刪除範圍時參考。
4. **架構**：兩個功能都沿用 ADR-6/ADR-7 已建立的「單次使用者可見的 LLM 呼叫先反問確認（設定 `pending_*` 狀態），下一輪由另一次**內部**（非使用者可見）LLM 呼叫做分類判斷，確定才真正執行寫入/刪除」模式，不另外發明新架構。`/clean-target-dialog` 的初始觸發改用 router 層 regex 判斷（`_CLEAN_TARGET_DIALOG_PATTERN`），因為主題是自由文字、無法用固定觸發詞集合窮舉；但「候選資料裡哪些跟主題相關」與「使用者是否確認」這兩步仍是內部 LLM 呼叫，比照 `/clean-all-dialog` 的 `CONFIRM`/`CANCEL` 判斷慣例。
5. **刪除方式**：`knowledge_base` 沒有 `deleted_at` 軟刪除欄位，`/clean-target-dialog` 對知識庫一律採**硬刪除**（`db.delete()`）；`conversation_logs` 仍沿用既有的軟刪除慣例（比照 FR-10），兩者刻意不同調，因為知識庫的資料量與寫入頻率遠低於對話紀錄，硬刪除不會有效能疑慮，也不需要為了保留「復原可能性」额外增加欄位。

**理由**：與既有的「伺服器端防線」慣例一致（例如 `auth.is_owner()` 判斷通關密碼免除），不信任 LLM 對權限範圍的自我判斷；兩輪反問確認架構已在 ADR-6/ADR-7/FR-10 驗證過可行且複雜度可控，重新發明新機制不符合「不過度工程」原則。

**後果**：
- `src/migrations/0012_add_label_to_knowledge_base.sql`：`ALTER TABLE knowledge_base ADD COLUMN label TEXT`
- `src/bot/knowledge.py`：新增 `save_knowledge(db, category, content, label=None, user_id=None)`（通用寫入，呼叫端負責權限把關）；`build_context()`／`get_persona_text()` 改用新增的 `_join_rows()` helper 串接**同一類別底下所有列**，不再只取 `rows[0]`——因為 Owner 現在可能透過對話往 `general_family`／`general_persona` 追加第二、第三筆資料
- `src/bot/chat.py`：新增 `_REQUEST_SAVE_MARKER = "【REQUEST_SAVE】"`；`handle_chat_message()` 新增 `is_owner` 現場判斷並傳入 `_build_prompt()`；prompt 依 `is_owner` 給不同範圍規則，回覆末尾標記偵測到後設定 `pending_save_knowledge_confirm` 狀態
- `src/bot/commands.py`：新增 `handle_save_knowledge_confirm_step()`（內部 LLM 分類 + 伺服器端強制 `category`）、`start_clean_target_dialog_confirm()`／`handle_clean_target_dialog_confirm_step()`（候選比對 + 確認 + 執行刪除）
- `src/bot/router.py`：新增 `_CLEAN_TARGET_DIALOG_PATTERN` regex；`handle_message()` 提早算出 `is_owner` 並在觸發 `/clean-target-dialog` 時傳入；`_dispatch_active_flow()` 新增 `pending_save_knowledge_confirm`／`pending_clean_target_dialog_confirm` 兩個分派
- `tests/bot/conftest.py`：`FakeCloudSQLClient` 新增 `delete()`
...
6. **下載/轉錄前先短路（2026-08-02 追加）**：Robin 問「如果卡在最終確認狀態時又發送一個新語音，這時會如何處理」，追問後發現初版實作是「先下載、轉錄，再讓 `commands.py` 依 `via_voice=True` 拒絕」——語音一定會被拒絕，卻還是先花了一次 Drive 上傳＋Groq 轉錄的額度，與 FR-14/FR-15「下載前先擋」的一貫原則不一致。修正 `handle_voice_message()`：在下載/轉錄之前，先查一次 `state_store`，若目前 flow 屬於 `_FINAL_CONFIRM_FLOWS`，直接呼叫 `handle_message(..., via_voice=True)`（`text` 帶空字串即可，因為這幾個 `handle_*_final_confirm_step()` 收到 `via_voice=True` 時根本不會用到 `text`），完全不下載、不上傳、不呼叫 Groq；這個短路檢查也排在 FR-14／FR-15 檢查之前，因為只要卡在最終確認狀態，語音必定被拒絕，沒必要再多做 FR-15 的資料庫查詢。
...
- [x] Step 19（2026-08-01，ADR-8）：`src/migrations/0012_add_label_to_knowledge_base.sql`：`knowledge_base` 新增 `label` 欄位（允許 NULL）
- [x] Step 20（2026-08-01，ADR-8）：`knowledge.py` 新增 `save_knowledge()`；`build_context()`／`get_persona_text()` 改用 `_join_rows()` 串接同類別所有列
- [x] Step 21（2026-08-01，FR-11／ADR-8）：`chat.py` 新增 `_REQUEST_SAVE_MARKER`／`is_owner` 參數，prompt 依 Owner／非 Owner 給不同存放範圍規則，並把 FR-3(g) 誠實性規則改為導向 `【REQUEST_SAVE】` 反問流程
...
- [x]（2026-08-01，FR-11／ADR-8）`chat._build_prompt()`：prompt 含 `【REQUEST_SAVE】` 規則與「分類/標籤」字樣；Owner 看到的規則允許選共用或個人範圍、非 Owner 只看到個人範圍規則；`handle_chat_message()` 回覆含 `【REQUEST_SAVE】` 標記時去除標記並設定 `pending_save_knowledge_confirm` 狀態（含 `target_user_id`／`original_request`）
