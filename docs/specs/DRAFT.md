...
- 2026-08-18：主選單「⏰ 排程設定」改名「⏰ 功能開關與排程設定」，新增 `tech_intel`／`job_search`／`certificate` 三項開關的選單化開啟/關閉操作（對應既有 FR-1／FR-2，目前這三項開關沒有 Telegram 選單入口）；已定案方向，實作排入下個階段，本階段（求職設定選單化，SPEC.md FR-41）不動 — 詳見 `docs/ADR/discuss/feature-toggles.md` 2026-08-18 條目
