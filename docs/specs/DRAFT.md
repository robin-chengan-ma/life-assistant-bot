...
- [ ] 2026-08-16：Robin 執行完整 `pytest tests/` 發現 16 項與 Phase 6 第二批 2c（心情、運動）相關的既有測試失敗（函式簽章不符／函式已在 2c 移除但測試沒跟著改），現況與初步比對見 `docs/ADR/debug/robinson.md` 2026-08-16「補述」段落；根因尚未排查，需要另外排時間處理，不在 2d 範圍內。
