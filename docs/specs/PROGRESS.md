...
| 「開始作答」批改結果與下一題拆成兩則訊息推播 | 完成（已 commit `52791a7`，尚未 push） | 不需 Migration。Robin 實機驗收上一項改版時回報批改結果（✅／❌＋詳解）跟下一題（或全部完成訊息）擠在同一則訊息一起推送；排查發現 `handle_quiz_answer_step()` 用 `f"{feedback}\n\n{next_prompt}"` 接在一起一次回傳。改成 `handle_quiz_answer_step()` 新增 `telegram_client` 參數，批改結果改為函式內直接呼叫 `telegram_client.send_text()` 立刻單獨送出，函式本身只回傳下一題／完成訊息，交給既有機制當第二則訊息送出。`src/bot/commands.py`、`src/bot/router.py` 已異動，`tests/bot/test_certificate_answer_commands.py`、`tests/bot/test_router.py` 同步更新。`pytest tests/bot -q` 1136 passed（另有 4 項既有 `test_job_search.py` 失敗屬雲端沙盒暫存快取版本較舊，與本次改動無關），`ruff check` 全過。Robin 同時反饋今天的題目全是單字題、沒有聽力題，這屬於題庫資料或每日題數設定（聽力/讀寫/單字比例）問題，非本次程式碼修正範圍，待 Robin 自行到「考試設定→每日題數設定→TOEIC」確認設定與題庫資料後續追蹤，見 `docs/ADR/discuss/robinson.md` 2026-09-06 續條目。 |
| 「開始作答」入口修正：移至主選單、修文字回覆路由缺口、跨證照標示 | 完成（已 commit `93ed786`，尚未 push） | 不需 Migration。Robin 回報回覆推播訊息承諾的「開始作答」沒有進入作答流程，排查發現 `router.py::handle_message()` 完全沒有涵蓋這句文字，會掉進一般聊天；改用按鈕時進一步發現按鈕藏在「考試設定→每日題數設定→選證照」很深的路徑，且該按鈕實際上不分證照、一次清完當天所有待答題目，放在單一證照頁面底下本身就誤導。三項一併修正：①主選單新增獨立項目「▶️ 開始作答」（`menu.py`），移除原本藏在深處的同名按鈕與對應 callback；②補上文字觸發詞「開始作答」；③作答畫面新增證照類型標示（「📝【證照類型】第 N/總題數 題」），解決跨證照切換看不出目前科目的問題。`src/bot/menu.py`、`src/bot/router.py`、`src/bot/certificate_settings.py`、`src/bot/certificate_answer.py`、`src/bot/commands.py` 已異動，`tests/bot/test_router.py` 同步更新／新增測試。`pytest tests/bot -q` 1136 passed（另有 4 項既有 `test_job_search.py` 失敗屬雲端沙盒暫存快取版本較舊，與本次改動無關），`ruff check` 全過。`docs/specs/SPEC.md` FR-27 同步更新。決策脈絡見 `docs/ADR/discuss/robinson.md` 2026-09-06 條目。 |
...
| 修正 `93ed786` 誤刪聽力題文字隱藏邏輯（洩題迴歸） | 完成（已 commit `abc4356`，尚未 push） | 不需 Migration。起因：Robin 反饋聽力題沒出現，Claude 排查過程一度誤判 ADR-32（聽力題目庫改版）三項功能未實作完成（實為分析用了雲端過期快照），Robin 追問「你確定現在的程式碼可以處理這些情境嗎」後，Claude 改用 `device_bash` 重新從 Robin 電腦拉取最新程式碼與測試，執行 `pytest tests/bot/test_certificate_answer.py` 發現 2 項失敗，`git log -p` 比對確認是本次 session 稍早的 `93ed786`（「開始作答」入口修正）改動 `_build_certificate_question_view()` 時誤刪了 ADR-32 決定的「聽力題不顯示文字題目/選項」分流邏輯，退化成一律顯示文字（洩題），`93ed786` 當時測試範圍選太窄沒跑到這支模組的既有單元測試才沒被抓到。`src/bot/certificate_answer.py`（restore `question_type == "listen"` 時 `prompt_lines = []` 的分流）已異動。`pytest tests/bot/test_toeic.py tests/bot/test_certificate_answer.py tests/bot/test_certificate_answer_commands.py tests/bot/test_router.py -q` 210 passed；全專案 `pytest tests/bot -q` 1143 passed（另有 4 項既有 `test_job_search.py` 失敗與本次改動無關），`ruff check` 全過。不需更新 SPEC（FR-27 本來就記載正確行為，這次是修復回歸，不是規格變更）。根因排查與經驗教訓見 `docs/ADR/debug/robinson.md` 2026-09-06 條目。 |

| 修正 `_split_whole_audio()` 先送整份音檔給 Groq 才裁切，導致 413 被拒 | 完成（已 commit `7e343e7`，尚未 push） | 不需 Migration。Robin 貼出 Render 正式環境錯誤 log，證實 9/6 週日 22:00 排程確實有觸發並掃到 `monster01` 這批聽力檔案，但 `voice_client.transcribe_with_segments()` 因 `413 Payload Too Large` 被 Groq 拒絕；排查發現 `_split_whole_audio()` 執行順序寫反——先把「整份沒剪過」的原始音檔送去 Groq 轉錄，`cutoff_seconds` 裁切要等轉錄成功回來才執行，完全沒有限制送出去的檔案大小，這是 ADR-32 實作當時就寫錯順序，不是後續迴歸。`src/bot/toeic.py`（`_split_whole_audio()` 改成有 `cutoff_seconds` 時先裁切、只把裁過的小檔送去轉錄）已異動。新增 `tests/bot/test_toeic.py::test_split_whole_audio_transcribes_trimmed_bytes_not_the_full_file_when_cutoff_set` 鎖定順序。`pytest tests/bot/test_toeic.py -q` 60 passed；全專案 `pytest tests/bot -q` 1144 passed（4 項既有 job_search 失敗與本次無關）；`ruff check` 通過。根因排查見 `docs/ADR/debug/robinson.md` 2026-09-06 續二條目。Robin 要求立刻重跑、不要等下週日；一開始新增 `scripts/manual_run_toeic_sync.py` 作為手動觸發工具，Robin 認為這種一次性工具腳本不該留在正式程式碼庫、要求 clean code，隨即移除該檔案（commit `8d72d60`），改為只在對話中提供 Render Shell 一次性指令，不留任何新增檔案在 repo 裡。 |

| 聽力整包音檔改用「Number N」題號標記切割，只存 100% 確定的題目、不確定就永久跳過不重試 | 完成（已 commit `abc708b`，尚未 push） | 新增 migration `0100_create_certificate_listen_split_failures_table.sql`。起因：Robin 本機重跑後回報「亂切」——`_find_split_plan()` 停頓長度啟發式猜測法在真實錄音（`monster01`）上 6 題全部猜錯。Robin 提出聽力錄音本來就會念題號（「Number 1」「Number 2」...），比停頓猜測可靠，遂新增依題號標記切割的邏輯；隨後 Robin 進一步明確要求「沒把握的題目就跳過，不要用猜的，儲存到資料庫的只能是 100% 確定的題目」，因此完全移除停頓長度猜測法當備援，改為逐題判斷：起始邊界＝自己的「Number N」標記，結束邊界＝下一題的標記或（若是本批最後一題）音檔／cutoff 結尾，任一邊界不確定就整題跳過。Robin 接著再要求「切不出來的題目下次排程不要再重跑，因為沒意義又浪費時間」——同一份錄音重轉錄結果基本固定，重試注定還是失敗；因此新增 `certificate_listen_split_failures` 表，把「確認放棄」的題目記錄下來，之後排程掃描到同一題直接跳過、完全不再下載/轉錄/切割（**永久跳過，不是下次自動重試**；真要重試需手動刪除該表紀錄）。`src/bot/toeic.py`（新增 `_NUMBER_WORDS`／`_NUMBER_MARKER_PATTERN`／`_find_number_marker_starts()`；`split_audio_by_question_count()` 改為逐題判斷邊界確定性；完全刪除 `_find_split_plan()`／`_segment_length_variance()`／`_split_points_for_target_length()` 及相關常數；新增 `_is_listen_split_already_failed()`／`_record_listen_split_failure()`；`_process_listen_questions()` 整合上述兩者；模組頂部 docstring 同步更新）已異動。`tests/bot/conftest.py` 新增假資料表與查詢條件支援。`tests/bot/test_toeic.py` 同步改寫（刪除停頓法專屬測試，新增標記解析、逐題邊界確定性、永久跳過不重試等測試）。`python3 -m pytest tests/bot/test_toeic.py -q` 59 passed；全專案 `python3 -m pytest tests/bot -q` 1143 passed（4 項既有 `test_job_search.py` 失敗與本次無關）；`ruff check` 通過。已同步更新 `docs/specs/SPEC.md`（FR-25b）、`docs/reference/db_schema.md`（新表結構）。決策脈絡見 `docs/ADR/discuss/robinson.md` 2026-09-07／2026-09-13（含再續）條目。`monster01` 實際切割結果已由 Robin 重跑後在資料庫實機驗收確認正確。 |

| 修正下載解答/題目照片沒包例外處理，暫時性網路問題會讓題目「憑空消失」 | 完成（已 commit `abc708b`，尚未 push） | 不需 Migration。起因：Robin 在本機用新邏輯重跑 `monster01` 驗證，跑到一半遇到 `IncompleteRead`（網路暫時中斷）整個腳本掛掉，重跑第二次後回報「缺前三題」，但這 3 題在 `certificate_questions` 與新增的 `certificate_listen_split_failures` 兩張表都查不到，跟 11、12、21、22（有明確記錄「找不到標記」）性質不同。排查發現 `_process_write_questions()`、`_process_listen_questions()`（2 處）、`_process_answer_keys()` 下載解答/題目照片那一行，從最初實作以來就沒有包 try/except（跟緊接在旁邊、有做例外處理的「上傳切割後音檔」那段待遇不一致），一遇到網路抖動就直接把整個 `sync_track1_from_drive()` 中斷，讓還沒處理到的題目連寫進資料庫或記錄失敗的機會都沒有，兩張表都查無資料。新增共用函式 `_download_file_or_none()` 把下載呼叫包一層例外處理，失敗記 log 並回傳 `None`，呼叫端跳過這一題即可，不拖累同一批次其他題目；這種失敗刻意不記錄進 `certificate_listen_split_failures`（那張表只給「切不出音檔邊界」這種確定性失敗用），沒寫入 `source_image_filename` 就會在下次同步自動重試，符合原本設計的重試語意。`src/bot/toeic.py` 四個下載呼叫點已改用新函式。新增 `tests/bot/test_toeic.py::test_sync_recovers_from_transient_download_failure_on_one_answer_photo`。`python3 -m pytest tests/bot/test_toeic.py -q` 60 passed；全專案 `python3 -m pytest tests/bot -q` 1144 passed（4 項既有 `test_job_search.py` 失敗與本次無關）；`ruff check` 通過。根因排查見 `docs/ADR/debug/robinson.md` 2026-09-14 條目。`monster01` 這次漏掉的第 1~3 題不會自動回填，已由 Robin 重新排程／手動重跑並確認補上。 |

| 修正 `GDriveClient.list_files()` 沒有處理 Google Drive API 分頁，資料夾檔案數超過單頁上限時後面幾頁會整個消失 | 完成（已 commit `abc708b`，尚未 push） | 不需 Migration。起因：上一列「下載例外處理」修復套用後，Robin 重跑同步指令回報「新增題目數：0，本次新記錄為『放棄不重試』的題目數：0」——代表 `monster01` 第 1～3 題根本沒有被嘗試處理過，不是處理途中失敗，推翻了原本「下載例外導致中斷」的完整解釋。先用截圖確認第 1～3 題來源檔案未被誤刪，排除誤刪可能；接著檢查 `submodules/gdrive/client.py` 的 `list_files()`，發現只呼叫一次 `files().list()`、完全沒有處理 `nextPageToken`，Google Drive API 單次呼叫預設最多回傳約 100 筆結果；經 Robin 確認資料夾內檔案數「超過 100 個」，確定這就是根因——不是抓到處理失敗，是連列表都沒列出來，每次重跑都 100% 重現。改成用 `while` 迴圈搭配 `nextPageToken`／`pageToken` 抓完所有分頁，`pageSize` 明確設為 API 上限 1000，直到某一頁沒有 `nextPageToken` 才停止；`fields` 同步加上 `nextPageToken`。`submodules/gdrive/client.py`（`list_files()`）已異動。`tests/submodules/gdrive/test_client.py` 新增 `test_list_files_follows_pagination_until_no_next_page_token`，並更新 `test_list_files_sends_correct_query_and_fields` 的 `fields`/`pageSize`/`pageToken` 斷言。`python3 -m pytest tests/submodules/gdrive/test_client.py -q` 21 passed；全專案 `python3 -m pytest tests/bot tests/submodules -q` 1321 passed（13 項失敗均為既有、與本次無關：4 項 `test_job_search.py`、9 項 `tests/submodules/email/test_client.py` 缺少 `smtplib` import 屬性）；`ruff check` 通過。已確認 `src/bot/commands.py`（2 處）與 `src/bot/toeic.py`（1 處）呼叫 `list_files()`，`fields` 變更純粹疊加不影響既有行為。根因排查見 `docs/ADR/debug/robinson.md` 2026-09-14「續」條目。`monster01` 第 1～3 題已由 Robin 重跑後在資料庫實機驗證，確認正確出現。 |
...
| 2026-09-06 | 「開始作答」入口修正 | Robin 回報回覆推播訊息承諾的「開始作答」掉進一般聊天，排查發現文字路由完全沒有涵蓋這句話；追查按鈕入口發現藏在很深的路徑且不分證照類型；改為主選單新增獨立項目、補上文字觸發詞、作答畫面加上證照類型標示三項一併修正 | Claude | 完成（已 commit `93ed786`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-06 條目。 |
| 2026-09-06 | 「開始作答」訊息拆分 | Robin 實機驗收後回報批改結果跟下一題擠在同一則訊息、答完最後一題的完成訊息也黏在批改結果底下；排查發現 `handle_quiz_answer_step()` 把兩者接成同一個字串一次回傳；改成批改結果透過 `telegram_client` 立刻單獨送出，函式只回傳下一題／完成訊息當第二則。同時記錄 Robin 反饋沒看到聽力題，判斷屬資料/設定問題非程式碼缺陷，待 Robin 自行確認 | Claude | 完成（已 commit `52791a7`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-06 續條目。 |
| 2026-09-06 | 聽力題排查與迴歸修復 | Robin 追問聽力題沒出現的原因；Claude 一度誤判 ADR-32 聽力題功能未實作完成（分析用了過期快照），Robin 追問「你確定現在的程式碼可以處理這些情境嗎」後改用 `device_bash` 拉最新程式碼＋測試，確認 `_cutoff` 切割與解答照片建題邏輯其實都在，但發現 `93ed786` 誤刪了聽力題「不顯示文字題目/選項」的分流邏輯，退化成洩題；已修復並補強測試涵蓋範圍 | Claude | 完成（已 commit `abc4356`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-06 條目。 |
| 2026-09-06 | 聽力整包音檔裁切順序修正 | Robin 貼出 Render 正式環境 413 錯誤 log；排查發現 `_split_whole_audio()` 先送整份未裁切音檔給 Groq 轉錄、`cutoff_seconds` 裁切要等轉錄成功才執行，完全沒發揮限制檔案大小的效果，是 ADR-32 實作當時順序就寫錯；改成先裁切再轉錄 | Claude | 完成（已 commit `7e343e7`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-06 續二條目。 |
| 2026-09-13 | 聽力題切割改用題號標記、不確定就跳過 | Robin 本機重跑回報整包音檔「全部都有」亂切；Robin 提出用聽力錄音本來就會念的「Number X」題號標記去切，比停頓長度猜測可靠，遂實作；Robin 進一步明確要求「沒把握的題目就跳過，不要用猜的，只存 100% 確定的題目」；完全移除停頓長度猜測備援，改為逐題判斷起始／結束邊界是否都 100% 確定 | Claude | 完成（已 commit `abc708b`，尚未 push） | 不需 Migration（本項）。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-07／2026-09-13 條目。 |
| 2026-09-13 | 切割失敗的題目改為永久跳過、不再自動重試 | Robin 追問「這批是之前跑過的還是新的」怎麼分辨後，進一步提出「切不出來的部分下次排程不要再重跑，因為沒意義又浪費時間」；排查確認同一份錄音重轉錄結果基本固定，重試注定還是失敗，於是新增 `certificate_listen_split_failures` 表記錄「已放棄」的題目，排程掃描時跳過已記錄的題號，不再下載/轉錄/切割；確認這是永久跳過，真要重試需手動刪除紀錄（Robin 確認「反正就是失敗的就不必再跑了」） | Claude | 完成（已 commit `abc708b`，尚未 push） | 新增 migration `0100_create_certificate_listen_split_failures_table.sql`。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-13 續條目、`docs/reference/db_schema.md` 新表結構。 |
| 2026-09-14 | 修正下載解答照片沒包例外處理，會讓題目「憑空消失」 | Robin 在本機用新邏輯重跑 `monster01` 驗證，途中遇到一次網路中斷（`IncompleteRead`）整個腳本掛掉，重跑第二次後回報「缺前三題」，但這 3 題在 `certificate_questions`／`certificate_listen_split_failures` 兩張表都查不到；排查發現 `_process_write_questions()`／`_process_listen_questions()`（2 處）／`_process_answer_keys()` 下載解答/題目照片那行從最初實作就沒包 try/except，一遇網路抖動就把整個 `sync_track1_from_drive()` 中斷，讓還沒處理到的題目連寫入或記錄失敗的機會都沒有；新增共用函式 `_download_file_or_none()` 包一層例外處理，下載失敗只跳過那一題、下次自動重試 | Claude | 完成（已 commit `abc708b`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-14 條目。 |
| 2026-09-14 | 修正 `GDriveClient.list_files()` 缺少分頁處理，資料夾檔案數超過單頁上限時後面幾頁完全消失 | Robin 套用上一項修復後重跑，回報「新增題目數：0，放棄不重試題目數：0」，代表第 1～3 題根本沒被嘗試處理過；先排除來源檔案被誤刪的可能，再排查發現 `list_files()` 只呼叫一次 API、沒有處理 `nextPageToken`，Drive API 單次呼叫預設最多回傳約 100 筆；Robin 確認資料夾「超過 100 個」檔案，確定為結構性、每次重跑都 100% 重現的分頁遺漏，改成迴圈抓完所有分頁；Robin 重跑後確認 monster01 第 1～3 題已正確出現在資料庫中 | Claude | 完成（已 commit `abc708b`，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-14「續」條目。 |
...
| --- | --- | --- | --- |
| 2026-09-14 | `abc708b` | 修正聽力題切割邏輯、下載例外處理與 Drive 分頁遺漏（4 項合併：Number N 標記切割、永久跳過不重試、下載例外處理、`list_files()` 分頁修正） | Claude |
| 2026-09-06 | `8d72d60` | 移除一次性手動觸發腳本，改用 Render Shell 一次性指令 | Claude |
| 2026-09-06 | `6ed3046` | 補回 7e343e7 commit hash 至 PROGRESS.md | Claude |
| 2026-09-06 | `7e343e7` | 修正整包聽力音檔先送 Groq 才裁切（413）；新增手動立即觸發腳本 | Claude |
| 2026-09-06 | `259f522` | 補回 abc4356 commit hash 至 PROGRESS.md | Claude |
| 2026-09-06 | `abc4356` | 修復聽力題文字隱藏邏輯迴歸（93ed786 誤刪） | Claude |
| 2026-09-06 | `52791a7` | 拆分作答批改結果與下一題為兩則獨立訊息 | Claude |
| 2026-09-06 | `93ed786` | 修正「開始作答」入口——移至主選單、補文字路由、加證照標示 | Claude |
