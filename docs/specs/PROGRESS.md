...
| 2026-09-06 | 聽力整包音檔裁切順序修正 | Robin 貼出 Render 正式環境 413 錯誤 log；排查發現 `_split_whole_audio()` 先送整份未裁切音檔給 Groq 轉錄、`cutoff_seconds` 裁切要等轉錄成功才執行，完全沒發揮限制檔案大小的效果，是 ADR-32 實作當時順序就寫錯；改成先裁切再轉錄 | Claude | 完成（新 commit，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-06 續二條目。 |
