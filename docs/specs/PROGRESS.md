...
| 2026-09-05 | 待辦事項提醒視窗 | Robin 追問 30 分鐘觸發頻率會不會漏推提醒；確認視窗＝頻率零緩衝的已知取捨依然存在，改為放寬 `_REMINDER_WINDOW` 為 40 分鐘、換取 10 分鐘容錯緩衝，並修正原本寫死「30 分鐘」的提醒文字改為動態計算實際剩餘分鐘數 | Claude | 完成（已 commit，尚未 push） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/discuss/infra.md` 2026-09-05 續條目。 |
