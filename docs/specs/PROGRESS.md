...
| 範圍 | 目前狀態 | 依據／剩餘工作 |
| --- | --- | --- |
| Telegram Phase 6 選單化、功能開關、排程、一般對話、FR-6c 草稿保護、FR-77 取消功能清理 | 完成（已 push／部署／實機驗收）；Mobile 殘留修正已 push、待部署與實機驗收確認 | 既有功能 commit `1601f34`；migration 修復 commit `07e986a`。Render 於 2026-08-18 12:25 依序成功套用 0084～0094，0094 已完成四張取消功能資料表清理；Mobile 客訴與舊錯誤結案死程式已由 commit `fb8c616` 移除並推送。 |
...
| Mobile 分析頁、目標摘要、成果置頂、求職與考試頁改版（FR-64b～FR-64d／FR-73a／FR-76b） | 第一、二批已 push 並實機驗收；第三、四批已 commit，待 push、部署與實機驗收 | commit `28ad29a`、`e667a11` 已 push 並實機測試。第三、四批 commit `83db1ec` 已統一修正七個分析頁首次載入、加入收藏目標摘要，並完成 Mobile／Telegram 共用成果置頂與 migration `0097`。求職及考試頁仍待後續批次。 |
...
| 2026-08-19 | FR-64b～FR-64d／FR-73a／FR-76b | Mobile 第三、四批：統一首次載入畫面、收藏目標摘要、Mobile／Telegram 跨端成果置頂與排序 | Codex | 已 commit／待 push、部署與 Robin 實機驗收 | commit `83db1ec`。Robin 已核准 migration `0097`；新增 nullable `pinned_at` 與有效成果排序索引，不回填或刪除既有資料。TDD RED：6 failed；GREEN：聚焦 53 passed。全專案 `pytest -q`：1806 passed、1 項第三方 warning；`ruff check .`、Mobile typecheck、Web export 通過。 |
