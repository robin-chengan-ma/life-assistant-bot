...
| 2026-08-24 | FR-41e | Robin 回報 Mobile App「推薦職缺」全部顯示「一分」且包含明顯不相關職缺，要求「找不到合適的就不要硬推薦」；排查發現 `GEMINI_API_JOB_SEARCH_KEY` 未設定導致 FR-37 評分管線從未執行、`score` 恆為 NULL，且推薦清單原本沒有任何分數門檻；與 Robin 討論後定案新增 60 分門檻，無符合項目時顯示「目前沒有符合的推薦職缺」 | Claude | 完成（已 commit `179e068`，尚未 push／部署） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/job-search.md` 2026-08-24「推薦職缺不分青紅皂白」條目。`GEMINI_API_JOB_SEARCH_KEY` 由 Robin 自行於 Render 補上。 |
