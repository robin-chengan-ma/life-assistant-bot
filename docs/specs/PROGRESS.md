...
| 2026-09-13 | 聽力題切割改用題號標記、不確定就跳過 | Robin 本機重跑回報整包音檔「全部都有」亂切；Robin 提出用聽力錄音本來就會念的「Number X」題號標記去切，比停頓長度猜測可靠，遂實作；Robin 進一步明確要求「沒把握的題目就跳過，不要用猜的，只存 100% 確定的題目」；完全移除停頓長度猜測備援，改為逐題判斷起始／結束邊界是否都 100% 確定 | Claude | 完成（待 commit） | 不需 Migration（本項）。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-07／2026-09-13 條目。 |
| 2026-09-13 | 切割失敗的題目改為永久跳過、不再自動重試 | Robin 追問「這批是之前跑過的還是新的」怎麼分辨後，進一步提出「切不出來的部分下次排程不要再重跑，因為沒意義又浪費時間」；排查確認同一份錄音重轉錄結果基本固定，重試注定還是失敗，於是新增 `certificate_listen_split_failures` 表記錄「已放棄」的題目，排程掃描時跳過已記錄的題號，不再下載/轉錄/切割；確認這是永久跳過，真要重試需手動刪除紀錄（Robin 確認「反正就是失敗的就不必再跑了」） | Claude | 完成（待 commit） | 新增 migration `0100_create_certificate_listen_split_failures_table.sql`。詳見上方「目前有效進度」列與 `docs/ADR/discuss/robinson.md` 2026-09-13 續條目、`docs/reference/db_schema.md` 新表結構。 |
| 2026-09-14 | 修正下載解答照片沒包例外處理，會讓題目「憑空消失」 | Robin 在本機用新邏輯重跑 `monster01` 驗證，途中遇到一次網路中斷（`IncompleteRead`）整個腳本掛掉，重跑第二次後回報「缺前三題」，但這 3 題在 `certificate_questions`／`certificate_listen_split_failures` 兩張表都查不到；排查發現 `_process_write_questions()`／`_process_listen_questions()`（2 處）／`_process_answer_keys()` 下載解答/題目照片那行從最初實作就沒包 try/except，一遇網路抖動就把整個 `sync_track1_from_drive()` 中斷，讓還沒處理到的題目連寫入或記錄失敗的機會都沒有；新增共用函式 `_download_file_or_none()` 包一層例外處理，下載失敗只跳過那一題、下次自動重試 | Claude | 完成（待 commit） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-14 條目。 |
| 2026-09-14 | 修正 `GDriveClient.list_files()` 缺少分頁處理，資料夾檔案數超過單頁上限時後面幾頁完全消失 | Robin 套用上一項修復後重跑，回報「新增題目數：0，放棄不重試題目數：0」，代表第 1～3 題根本沒被嘗試處理過；先排除來源檔案被誤刪的可能，再排查發現 `list_files()` 只呼叫一次 API、沒有處理 `nextPageToken`，Drive API 單次呼叫預設最多回傳約 100 筆；Robin 確認資料夾「超過 100 個」檔案，確定為結構性、每次重跑都 100% 重現的分頁遺漏，改成迴圈抓完所有分頁 | Claude | 完成（待 commit） | 不需 Migration。詳見上方「目前有效進度」列與 `docs/ADR/debug/robinson.md` 2026-09-14「續」條目。 |
