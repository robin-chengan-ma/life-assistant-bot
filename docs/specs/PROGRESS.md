...
| 2026-08-15 | FR-77 | 定案取消功能的路由、流程、測試與資料表清理 | Codex | 部分完成／待續 | 第一批淘汰 `complaints`、`knowledge_base`、`conversation_logs`、`conversation_summaries`；一般聊天正式路由已停止持久化，其餘客訴路由、舊函式、相關測試及四張資料表仍待清理。NFR-14～NFR-15 架構遷移已於 2026-08-18 取消，不再列入 Roadmap |
...
| 2026-08-15 | FR-6c | 定案 Telegram 功能模式切換、10 分鐘逾時、草稿保護與功能名稱確認入口 | Codex | 已定案／待開發 | 2026-08-18 補充確認：模式逾時後草稿保留至 30 分鐘且期間可一般聊天；只有已有輸入的新增／編輯流程算草稿，每位使用者每個功能最多一份且可跨功能並存；再次進入原功能須先顯示草稿摘要並提供繼續／放棄；自然語言採固定名稱／別名，只導向選單、不使用 LLM 猜測或直接異動資料。權限檢查套用選單、Callback、文字／語音名稱偵測與模式切換 |
