...
- [x] FR-9（2026-08-02 新增，見 ADR-10）：`submodules/gdrive` 改用 OAuth 2.0（以真人 Google 帳號身分）認證，不再使用 Service Account；`GDriveClient` 建構子改為 `refresh_token`／`client_id`／`client_secret`／`folder_id` 四個必要參數。**2026-08-07 追記（見 robinson SPEC.md Step 3.2、ADR-18）**：新增 `list_files()`／`download_file()`，scope 從單純 `drive.file` 擴大為 `drive.file + drive.readonly`（TOEIC 題庫 Pipeline 需要掃描/讀取 Robin 手動上傳、非本程式建立的檔案），既有 refresh token 需重新走一次 `get_refresh_token.py` 才有新 scope
- [x] FR-8 追記（2026-08-07，見 robinson SPEC.md Step 3.2）：`submodules/voice` 新增 `transcribe_with_segments()`，用 `response_format=verbose_json`＋`timestamp_granularities[]=segment` 取得逐句時間軸，供 TOEIC 整包聽力 MP3 依語句停頓自動切割使用
...
- [x] Step S.7：建立 `submodules/gdrive/`（`client.py`、`README.md`、`requirements.txt`、`.env.example`），Step 1.3b 影像辨識需要（2026-07-31）——刻意只暴露 `upload_file()`，不做下載/列表/刪除，避免建置用不到的能力。**2026-08-07 追記**：Step 3.2 TOEIC 題庫 Pipeline 需要掃描 Drive 資料夾，補上 `list_files()`／`download_file()`，見上方 FR-9 追記
...
| 2026-08-07 | **Robin 選定 Step 3.2（TOEIC 雙軌題庫 Pipeline）開工，見 robinson SPEC.md ADR-18**：經 AskUserQuestion 確認三個設計問題（詳見 ADR-18）：① Robin 直接把照片/音檔手動丟進 Drive 資料夾＋機器人排程掃描（而非透過 Telegram），因此 `gdrive` 新增 `list_files()`／`download_file()`，scope 從 `drive.file` 擴大為 `drive.file + drive.readonly`（既有 refresh token 需重新走一次 `get_refresh_token.py`），見上方 FR-9 追記；20 個測試（含新增的 6 個），覆蓋率 100% ② 整包聽力 MP3 自動切割這次一起做，`voice` 新增 `transcribe_with_segments()`（`response_format=verbose_json`），13 個測試，覆蓋率 100% ③ Step 3.2 範圍只到「建題庫」，不含推播/作答。同步新增根目錄 `requirements.txt` 的 `pydub`（音訊切割，依賴系統 `ffmpeg`，Dockerfile 同步新增） | Claude（依 Robin 於 AskUserQuestion 確認範圍後實作） |
| 2026-08-08 | **Production 事故修復：`CloudSQLClient.execute()`／`execute_query()` 的 `IndexError` bug（根因追查）**。Robin 回報 Phase 2／3 的 migration 完全沒套用到 Neon（`monthly_budget`／`body_goals` 等欄位/表都不存在），Robin 提供 Render 開機 log 後確認卡在 `0018_add_budget_fields_to_users.sql`，錯誤是 `cursor.execute(query, params or ())` 的 `IndexError: tuple index out of range`。根因：舊版 `execute()`／`execute_query()` 只要 `params` 是 `None`，就用 `params or ()` 退回一個空 tuple 傳給 `cursor.execute()`；但 psycopg2 只要收到「非 None」的第二個參數（即使是空 tuple），就會對整個 query 字串套用 Python `%`-style 格式化解析——`0018` 的 `COMMENT ON COLUMN` 註解文字含字面 `%`（「50%」「80%」），被誤判成參數佔位符、找不到對應參數值就丟出 `IndexError`；`grep -l '%' src/migrations/*.sql` 找出另一個之後也會踩到的檔案（`0034_split_skill_growth_toggle.sql`，含 `LIKE '%...%'` 與 PostgreSQL `format()` 的 `%I`）。修正：`params is None` 時完全不帶第二個參數呼叫 `cursor.execute(query)`，讓 psycopg2 不解析 `%`；`select`/`insert`/`update`/`delete` 因為一律有明確參數，不受影響。新增／改寫 6 個測試（含用真實含 `%` 字元的 SQL 字串驗證呼叫參數正確），全專案 958 個測試全過 | Claude（依 Robin 提供的 Render 開機 log traceback 找出根因並修復） |
