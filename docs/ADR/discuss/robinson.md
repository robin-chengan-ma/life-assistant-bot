...
**狀態**：deprecated（2026-08-18 僅保留 FR-77 取消功能清理；後端分層與目錄遷移決策已取消，見下方新決策）
...
**狀態**：deprecated（2026-08-18 架構與目錄責任決策已取消，見下方新決策）
...
### 2026-08-18 [標籤：使用者] 取消 NFR-14～NFR-15 架構遷移

**狀態**：accepted（supersedes 2026-08-15「淘汰路由、資料表與後端分層重構」中的後端分層／目錄遷移部分、「目錄責任邊界補充」，以及「選單矩陣、統一對話流程與重構執行順序」中的目錄搬遷順序；FR-77 清理決策與其他產品行為維持有效）

**背景**：Telegram 多項功能已完成正式環境實測。將 `main.py`／`src/` 大量搬至 `backend/`，再拆分 `api`、`services`、`repositories`、`agents` 與 `data`，改動面廣且可能使已驗收功能發生回歸。

**討論內容**：使用者要求徹底移除第三項 NFR-14～NFR-15 架構遷移，本專案不再執行這項重構；FR-6c 模式逾時／草稿保護／自然語言入口與 FR-77 取消功能清理仍保留。

**決策**：維持目前根目錄 `main.py`、`src/`、`mobile/` 與 `submodules/` 的程式結構及既有部署入口。不建立以搬遷為目的的 `backend/`、`backend/agents/` 或 `data/`，不再把既有 Telegram、Gemini、爬蟲或 ETL 程式大規模跨目錄拆遷。未來功能仍應在現有結構內維持合理職責分離，但不得以 NFR-14～NFR-15 名義啟動架構搬遷。

**理由**：保護已通過實機驗收的功能，避免只為目錄整齊而承擔大範圍回歸與部署風險。

**後果**：SPEC 移除架構版 NFR-14～NFR-15，PROGRESS 不再列為待開發 Roadmap，DRAFT「已取消」保留取消紀錄；此決策不取消 FR-77，也不授權刪除任何程式、測試或資料表。

### 2026-08-18 [標籤：使用者] FR-6c 草稿恢復與自然語言入口補充

**狀態**：accepted（補充 2026-08-15「一般對話與選單功能模式切換」及「選單矩陣、統一對話流程與重構執行順序」的草稿恢復細節）

**背景**：既有規格已定案功能模式 10 分鐘逾時、草稿保留 30 分鐘及切換功能三選一，但尚未明確規範逾時期間的一般聊天、再次進入原功能時的畫面，以及自然語言入口能否直接執行資料異動。

**討論內容**：使用者確認逾時後仍可一般聊天；再次進入有草稿的功能必須顯示草稿內容；自然語言只能導向選單功能。

**決策**：①功能模式逾時 10 分鐘後切回一般對話，但未送出草稿繼續保留至第 30 分鐘，期間一般聊天不覆蓋草稿。②再次進入仍有有效草稿的原功能時，先顯示草稿摘要，提供「繼續編輯／放棄草稿」，不得直接建立新草稿或覆蓋舊內容。③只有已輸入資料但尚未送出的新增／編輯流程算草稿；主選單、唯讀查詢、通關密碼及語音確認等暫存狀態不算草稿。④每位使用者可同時保留不同功能的草稿，但每個功能最多一份，各自依最後操作時間計算 30 分鐘。⑤自然語言入口採固定功能名稱與別名比對；明確命中時先詢問並引導至對應選單，未命中或無法確定時請使用者從主選單選擇，不使用 LLM 猜測功能，也不得直接新增、修改或刪除正式資料。

**理由**：避免逾時或功能切換造成輸入遺失，同時確保所有正式異動仍經過既有選單、驗證、摘要與二次確認流程。

**後果**：FR-6c 實作須將作用中模式與按功能保存的草稿分開管理，定義哪些新增／編輯狀態已形成有效草稿，並為多功能草稿到期、草稿恢復、固定別名導向及權限檢查補齊測試；本次只完成文件定案，尚未開工。
...
## 2026-08-18 [標籤：使用者] FR-6c 與 FR-77 開工執行紀錄

**狀態**：accepted（程式與 `0094` migration 已部署並完成實機驗收）

**背景**：Robin 在草稿保護細節定案後指示開發，並要求已取消的客訴、持久化知識庫與長期對話記憶相關路由、排程、資料表、測試與程式全面清理。

**執行內容**：功能狀態儲存拆分為 10 分鐘作用中模式與每功能獨立 30 分鐘草稿；既有新增／編輯 flow 由中央狀態層辨識，跨功能提供保留並切換／放棄並切換／繼續編輯，重新進入提供草稿摘要與繼續／放棄。自然語言入口只比對固定功能名稱與別名，需先按確認才開啟選單。FR-77 已移除舊 Bot 模組、Router 入口、Mobile 客訴分析入口與相關測試。
...
**部署驗證**：Render 於 2026-08-18 12:25 依序完成 migration `0084`～`0094`，服務正常啟動；`0094` 已完成四張取消功能資料表清理，Robin 後續完成 Telegram 與 Mobile 實機驗收。
...
### 2026-08-24 補充：🎯 目標追蹤體態摘要改綜合評估（只限體重目標）

**狀態**：accepted

**背景**：Robin 反饋「體態目標的摘要目前應該只看體重資料，但專業健身教練應該綜合飲食和運動紀錄評估才有實用性」。查 `src/services/goal_summary_job.py::_gather_body_activity_text()`，確認邏輯依 `goal_type` 分流，`weight` 目標只撈體重紀錄、`exercise` 只撈運動紀錄、飲食目標只撈飲食紀錄，各自完全獨立，符合 Robin 反饋的現象。追問後 Robin 進一步釐清：運動／飲食目標本身就是在追蹤「這件事有沒有做到」，不確定是否也需要看其他資料。

**決策**：只有 `goal_type == "weight"` 的目標改成同時撈體重／運動／飲食三項近期紀錄（一週／一個月）餵給 LLM，Prompt 追加「若紀錄包含多項資料，請像專業教練一樣綜合評估、指出彼此關聯」；`exercise`／飲食型目標維持原本只看自己那一種紀錄，不擴大範圍。

**理由**：體重是運動與飲食習慣共同作用下的「結果」，只看體重數字給不出「為什麼」，需要同時參考運動與飲食才能給出「這週體重沒降，但飲食紀錄熱量偏高」這類具體建議；運動／飲食目標本身就是行為追蹤型目標，重點是該行為本身的頻率／量是否達標，體重或另一項資料不是這類目標的評估依據，維持單一資料來源更聚焦、Prompt 也更精簡。
