...
**狀態**：superseded by 2026-08-15「移除持久化知識庫與對話記憶」決策
...
**狀態**：superseded by 2026-08-15「使用規則選單與功能總覽移除」決策
...
**狀態**：deprecated（2026-08-15 已取消查無答案後教學與知識庫寫入流程）
...
**狀態**：deprecated（2026-08-15 已取消持久化知識庫與相關問答流程）
...
**狀態**：deprecated（2026-08-15 已取消主動新增、刪除知識與清除對話流程）
...
**狀態**：deprecated（適用的三項高風險流程已於 2026-08-15 全數取消）
...
**狀態**：deprecated（2026-08-15 已取消清除對話入口）
...
## 2026-08-18 [標籤：使用者] 一般對話正式責任與取消功能文件分流

**狀態**：accepted

**背景**：`SPEC.md` 的 Gemini 對話核心仍以「FR（取消）」混列過時能力，且 API Reference 仍將舊知識庫、功能總覽與清除對話流程描述為一般對話正式能力，容易誤導後續開發。

**決策**：SPEC 只保留目前有效的一般對話責任：本人結構化資料唯讀查詢、使用者提供內容的整理分析、功能導引、10 分鐘記憶體上下文及媒體防呆。查無答案後教學、家庭／個人知識庫、逐則對話落地、長記憶摘要、`/function`、主動新增／刪除知識及清除對話等取消內容集中移至 `DRAFT.md`「已取消」，歷史原因保留於本 ADR。Reference 必須明確區分「現行舊程式仍存在」與「正式規格已取消／待 Phase 6 移除」。

**理由**：SPEC 是目前有效需求，混放取消行為會讓實作者誤把歷史功能繼續保留；DRAFT 與 ADR 分別承擔取消索引及歷史脈絡，同時 Reference 不能在程式尚未清除前虛構已不存在。

**後果**：Phase 6 實作仍須依 FR-77 移除舊路由、狀態、排程、測試及資料表；資料表 DROP 前的依賴、備份、回滾盤點與 Robin 二次確認規則不變。
