...
2. 語音長度限制、修正窗口限制等商業邏輯（見 SPEC.md FR-14／FR-15）一律由呼叫端（`src/bot/`）決定，本模組只負責「把 bytes 丟給 Groq 換回文字」。
3. 刻意不安裝官方 `groq` SDK：Whisper 轉錄端點是單純的 multipart POST，用 `requests` 直接呼叫即可，避免多一個重量依賴（比照 `submodules/telegram` 的作法）。

## 對應 Spec

[docs/specs/SPEC.md](../../docs/specs/SPEC.md) FR-14／FR-15、[docs/ADR/discuss/submodules-core.md](../../docs/ADR/discuss/submodules-core.md) ADR-12
